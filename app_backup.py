...
app = Flask(__name__)
# Bu anahtarı kimseyle paylaşma. Uygulamanın güvenliği için çok önemli.
app.secret_key = '0d321d58e37033503f268a8656113b28'
app.permanent_session_lifetime = timedelta(days=30)

# Google Login Ayarları
GOOGLE_CLIENT_ID = "1005593099445-ihljicun171qjul5m806qbk1jod95045.apps.googleusercontent.com" # Buraya kendi Client ID'ni yapıştır
GOOGLE_CLIENT_SECRET = "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" # Buraya kendi Client Secret'ini yapıştır
...
# E-POSTA AYARLARI
SENDER_EMAIL = 'listoria44@gmail.com'
SENDER_PASSWORD = 'jnfz whwn xhgb lqgo'
...
def kitap_oneri_al():
    ...
    # --- YENİ KONTROL EKLE ---
    # Veritabanındaki kitap başlıklarını al
    tum_kitaplar = get_all_books_database()
    kitap_basliklari = [k['baslik'].lower() for k in tum_kitaplar]
    # Girilen kitaplardan veritabanında olanları filtrele
    valid_kitaplar = [k for k in kullanici_kitaplari if k.lower() in kitap_basliklari]
    if len(valid_kitaplar) < 3:
        return render_template('kitap_oneri.html', hata="En az 3 geçerli kitap girmelisiniz.", yas=None, son_arama={})
    # --- KONTROL SONU ---
    ...
    try:
        oneriler = generate_book_recommendations(valid_kitaplar, yas, tur, min_sayfa, max_sayfa, notlar)
    ...
    return render_template('kitap_sonuc.html', 
                         oneriler=oneriler,
                         kullanici_kitaplari=valid_kitaplar,
                         yas=yas)
# ...existing code...
...
def generate_music_recommendations(kullanici_sarkilari, yas, dil, tur, notlar):
    tum_oneriler = get_all_music_database()
...
def generate_spotify_playlist(kullanici_sarkilari, dil, tur, notlar):
    # Spotify playlist linki oluştur (simüle edilmiş)
    # Gerçek uygulamada Spotify API kullanılır
    
    # Öneri şarkıları al
    oneriler = generate_music_recommendations(kullanici_sarkilari, None, dil, tur, notlar)
    
    # Playlist için şarkı listesi oluştur
    playlist_sarkilari = kullanici_sarkilari + [f"{sarki['baslik']} - {sarki['sanatci']}" for sarki in oneriler[:20]]
    
    # Simüle edilmiş Spotify playlist linki
    playlist_id = "listoria_" + str(hash(''.join(playlist_sarkilari)))[-8:]
    playlist_link = f"https://open.spotify.com/playlist/{playlist_id}"
    
    return {
        'link': playlist_link,
        'sarkilar': playlist_sarkilari,
        'isim': f"Listoria - {dil.title()} Müzik Karışımı"
    }

def calculate_music_similarity_scores(sarkilar, kullanici_sarkilari, notlar):
    scored_sarkilar = []
    
    for sarki in sarkilar:
        score = 0
        
        # Ek notlara göre skorlama (en önemli faktör)
        if notlar:
            notlar_lower = notlar.lower()
            for tema in sarki['tema']:
                if tema.lower() in notlar_lower:
                    score += 15
            
            # Sanatçı tarzı eşleşmesi
            if sarki['sanatci_tarzi'].replace('_', ' ') in notlar_lower:
                score += 10
        
        # Rastgele çeşitlilik için base score
        import random
        score += random.randint(1, 5)
        
        scored_sarkilar.append({**sarki, 'score': score})
    
    # Skora göre sırala
    scored_sarkilar.sort(key=lambda x: x['score'], reverse=True)
    return scored_sarkilar
